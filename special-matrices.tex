    ...
    \node (Matrix) {Matrix}
				child { node (Symmetric) {Symmetric}
						...
						child { node (Diagonal) {Diagonal}[sibling distance=8em]
								...
								child { node (EigenvalueMatrix) {Eigenvalue Matrix} }
						}
				}
				child { node (Invertible) {Invertible} }
				child { node [xshift=-3cm](Orthogonal) {Orthogonal}[sibling distance=5em]
						child { node [xshift=-0.5cm](LeftOrthogonal) {Left Orth} }
						child { node [xshift=0.5cm](RightOrthogonal) {Right Orth} }
				};
