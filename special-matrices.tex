...
\begin{longtable}{|>{\bfseries}m{3.5cm}|m{5cm}|m{10.5cm}|}
		...
		\hline
		Positive Definite & $\bx^T A \bx > 0$ for all non-zero $\bx$ &
		- All eigenvalues are positive \newline
		- $\det(A) > 0$ \newline
		- Invertible \\
		\hline
		Positive Semi-Definite & $A = A^T$, $\bx^T A \bx \ge 0$ for all $\bx$ &
